...
enum WatchConfig {
    ...
    private static var secrets: [String: Any]? = {
        guard let url = Bundle.main.url(forResource: "Secrets", withExtension: "plist") else {
            print("WatchConfig: Secrets.plist not found in bundle")
            return nil
        }
        guard let data = try? Data(contentsOf: url) else {
            print("WatchConfig: Could not read Secrets.plist data")
            return nil
        }
        guard let plist = try? PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any] else {
            print("WatchConfig: Could not parse Secrets.plist")
            return nil
        }
        print("WatchConfig: Secrets.plist loaded successfully")
        return plist
    }()
